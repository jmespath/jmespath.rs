...
use crate::{Context, JmespathError, ErrorReason, Rcvar, RuntimeError};
use crate::interpreter::{interpret, SearchResult};
use crate::variable::{Variable, JmespathType};
...
impl Function for CustomFunction {
    fn evaluate(&self, args: &[Rcvar], ctx: &mut Context) -> SearchResult {
        r#try!(self.signature.validate(args, ctx));
        (self.f)(args, ctx)
    }
}
...
impl Signature {
    ...
    pub fn validate(&self, args: &[Rcvar], ctx: &Context) -> Result<(), JmespathError> {
        r#try!(self.validate_arity(args.len(), ctx));
        if let Some(ref variadic) = self.variadic {
            for (k, v) in args.iter().enumerate() {
                let validator = self.inputs.get(k).unwrap_or(variadic);
                r#try!(self.validate_arg(ctx, k, v, validator));
            }
        } else {
            for (k, v) in args.iter().enumerate() {
                r#try!(self.validate_arg(ctx, k, v, &self.inputs[k]));
            }
        }
        Ok(())
    }
    ...
    ($ctx:expr, $operator:ident, $args:expr) => (
        {
            ...
            let initial = r#try!(interpret(&vals[0], &ast, $ctx));
            ...
            for (invocation, v) in vals.iter().enumerate().skip(1) {
                let mapped = r#try!(interpret(v, &ast, $ctx));
...
impl Function for MapFn {
    fn evaluate(&self, args: &[Rcvar], ctx: &mut Context) -> SearchResult {
        ...
        let mut results = vec![];
        for value in values {
            results.push(r#try!(interpret(&value, &ast, ctx)));
        }
...
impl Function for SortByFn {
    fn evaluate(&self, args: &[Rcvar], ctx: &mut Context) -> SearchResult {
        ...
        let first_value = r#try!(interpret(&vals[0], &ast, ctx));
        ...
        for (invocation, v) in vals.iter().enumerate().skip(1) {
            let mapped_value = r#try!(interpret(v, &ast, ctx));
