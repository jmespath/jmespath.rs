...
pub fn interpret(data: &Rcvar, node: &Ast, ctx: &mut Context) -> SearchResult {
    match *node {
        ...
        Ast::Subexpr { ref lhs, ref rhs, .. } => {
            let left_result = r#try!(interpret(data, lhs, ctx));
        ...
        Ast::Or { ref lhs, ref rhs, .. } => {
            let left = r#try!(interpret(data, lhs, ctx));
            if left.is_truthy() {
                Ok(left)
            } else {
        ...
        Ast::Not { ref node, .. } => {
            let result = r#try!(interpret(data, node, ctx));
        ...
        Ast::Condition { ref predicate, ref then, .. } => {
            let cond_result = r#try!(interpret(data, predicate, ctx));
        ...
        Ast::Comparison { ref comparator, ref lhs, ref rhs, .. } => {
            let left = r#try!(interpret(data, lhs, ctx));
            let right = r#try!(interpret(data, rhs, ctx));
        ...
        Ast::ObjectValues { ref node, .. } => {
            let subject = r#try!(interpret(data, node, ctx));
            match *subject {
        ...
        Ast::Projection { ref lhs, ref rhs, .. } => {
            match r#try!(interpret(data, lhs, ctx)).as_array() {
                None => Ok(Rcvar::new(Variable::Null)),
                Some(left) => {
                    let mut collected = vec![];
                    for element in left {
                        let current = r#try!(interpret(element, rhs, ctx));
                        if !current.is_null() {
        ...
        Ast::Flatten { ref node, .. } => {
            match r#try!(interpret(data, node, ctx)).as_array() {
                None => Ok(Rcvar::new(Variable::Null)),
                Some(a) => {
        ...
        Ast::MultiList { ref elements, .. } => {
            if data.is_null() {
                ...
                for node in elements {
                    collected.push(r#try!(interpret(data, node, ctx)));
                }
                Ok(Rcvar::new(Variable::Array(collected)))
            }
        }
        Ast::MultiHash { ref elements, .. } => {
            if data.is_null() {
                ...
                for kvp in elements {
                    let value = r#try!(interpret(data, &kvp.value, ctx));
        ...
        Ast::Function { ref name, ref args, offset } => {
            ...
            for arg in args {
                fn_args.push(r#try!(interpret(data, arg, ctx)));
            }
