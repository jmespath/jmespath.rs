...
pub fn parse(expr: &str) -> ParseResult {
    let tokens = r#try!(tokenize(expr));
...
impl<'a> Parser<'a> {
    ...
    fn expr(&mut self, rbp: usize) -> ParseResult {
        ...
        while rbp < self.peek(0).lbp() {
            left = self.led(Box::new(r#try!(left)));
        }
        left
    }
        ...
        match token {
            ...
            Token::Lbrace => {
                ...
                loop {
                    ...
                    pairs.push(r#try!(self.parse_kvp()));
                    match self.advance() {
        ...
        match token {
            t @ Token::Dot => {
                if self.peek(0) == &Token::Star {
                    ...
                    let offset = offset;
                    let rhs = r#try!(self.parse_dot(t.lbp()));
                    Ok(Ast::Subexpr {
                        offset: offset,
                        lhs: left,
                        rhs: Box::new(rhs),
                ...
                match match self.peek(0) {
                    ...
                    true => {
                        Ok(Ast::Subexpr {
                            offset: offset,
                            lhs: left,
                            rhs: Box::new(r#try!(self.parse_index())),
                        })
                    }
                    false => {
                        self.advance();
                ...
                match *left {
                    Ast::Field { name: v, .. } => {
                        Ok(Ast::Function {
                            offset: offset,
                            name: v,
                            args: r#try!(self.parse_list(Token::Rparen)),
                        })
                    }
    ...
    fn parse_filter(&mut self, lhs: Box<Ast>) -> ParseResult {
        ...
        let condition_lhs = Box::new(r#try!(self.expr(0)));
        // Eat the closing bracket.
        match self.advance() {
            Token::Rbracket => {
                let condition_rhs = Box::new(r#try!(self.projection_rhs(Token::Filter.lbp())));
                Ok(Ast::Projection {
                    offset: self.offset,
                    lhs: lhs,
    ...
    fn parse_flatten(&mut self, lhs: Box<Ast>) -> ParseResult {
        let rhs = Box::new(r#try!(self.projection_rhs(Token::Flatten.lbp())));
        Ok(Ast::Projection {
            offset: self.offset,
    ...
    fn parse_comparator(&mut self, cmp: Comparator, lhs: Box<Ast>) -> ParseResult {
        let rhs = Box::new(r#try!(self.expr(Token::Eq.lbp())));
        Ok(Ast::Comparison {
            offset: self.offset,
            comparator: cmp,
            lhs: lhs,
    ...
    fn parse_wildcard_index(&mut self, lhs: Box<Ast>) -> ParseResult {
        match self.advance() {
            Token::Rbracket => {
                let rhs = Box::new(r#try!(self.projection_rhs(Token::Star.lbp())));
                Ok(Ast::Projection {
                    offset: self.offset,
                    lhs: lhs,
                    rhs: rhs,
    ...
    fn parse_index(&mut self) -> ParseResult {
        ...
        if pos == 0 {
            ...
            Ok(Ast::Projection {
                ...
                rhs: Box::new(r#try!(self.projection_rhs(Token::Star.lbp()))),
            })
        }
    }
    ...
    fn parse_multi_list(&mut self) -> ParseResult {
        Ok(Ast::MultiList {
            offset: self.offset,
            elements: r#try!(self.parse_list(Token::Rbracket)),
        })
    }
