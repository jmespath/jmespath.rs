...
use super::Context;
use super::errors::{Error, ErrorReason, RuntimeError};
...
use super::functions::FnRegistry;
use super::Variable;
...
pub fn compile_filter(ast: &Ast, ctx: &mut Context) -> Result<Box<Filter>, Error> {
    match *ast {
        ...
        Ast::Function { ref name, ref args, offset } => {
            ctx.offset = offset;
            match ctx.fn_registry.get_function(name) {
                None => {
...
mod tests {
    ...
    use Context;
    use Variable;
    use DEFAULT_FN_REGISTRY;
    use streaming::listener::StringListener;
    ...
    pub fn run_test_cases(cases: Vec<(&str, &str, &str)>) {
        for (expr, json, result) in cases {
            ...
            let mut ctx = Context::new(expr, &*DEFAULT_FN_REGISTRY);
